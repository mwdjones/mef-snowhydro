...
```{r}
s2lai = read.csv("./Cleaned Data/S2_winterLAI_calibrated.csv")
```
...
```{r}
#Merge snow data
s2snow = data.frame(snow_avg)

s2lai_merge = merge(s2snow, s2lai, by.x = "stakes", by.y = "Stake_ID", all.y = TRUE)


rownames(s2lai_merge) = s2lai_merge$stakes
...
allData = merge(s2lai, merged_data, by.x = "Stake_ID", by.y = "SITE")
head(allData)
```
