...
```{r}
fig_save = "D:/1_DesktopBackup/Feng Research/0_MEF Snow Hydology/mef-snowhydro/Figures/Veg/"
load_path = "D:/1_DesktopBackup/Feng Research/0_MEF Snow Hydology/mef-snowhydro/Data and Codes/Cleaned Data/"
```
...
```{r}
#Import Data
forestInvS2 = readxl::read_excel("D:/1_DesktopBackup/Feng Research/0_MEF Snow Hydology/mef-snowhydro/Data and Codes/Cleaned Data/S2overstory_2023_compiled.xlsx",
                                col_names = TRUE,
                                col_types = c('text', 'text', 'text', 'numeric', 'numeric', 'numeric', 'numeric', 'text', 'numeric', 'numeric'), 
                                na = c("NAN", "NA"))
forestInvS6 = readxl::read_excel("D:/1_DesktopBackup/Feng Research/0_MEF Snow Hydology/mef-snowhydro/Data and Codes/Cleaned Data/S6overstory_2023_compiled.xlsx",
                                col_names = TRUE,
                                col_types = c('text', 'text', 'text', 'numeric', 'numeric', 'numeric', 'numeric', 'text', 'numeric', 'numeric'), 
                                na = c("NAN", "NA"))
...
```
 ...
 ggsave(paste0(fig_save, "canopyClassDistribution.png"), width = 5, height = 5)
 ```
...
#S6
 ...
 forestInvS2 = forestInvS2 %>% 
    ...
    mutate(DBH_CM_SD = sd(DBH_CM, na.rm=FALSE)) %>%
    ungroup()

 #DBH distribution - S2
 ggplot(data = forestInvS2, aes(x = DBH_CM, color = Zone, fill = Zone)) +
   geom_density() + 
   facet_grid(rows = vars(eastingBin_f), cols = vars(northingBin)) + 
   scale_color_pomological() +
   scale_fill_pomological() +
   xlim(0, 65) + 
   geom_text(mapping = aes(label = round(DBH_CM_SD, 2), x = 50, y = 0.15), size = 5) +
 ...
 ggsave(paste0(fig_save, "S2DBHDistribution.png"), width = 10, height = 7)
```
...
```{r}
 ...
 #DBH distribution - S6
 ggplot(data = forestInvS6, aes(x = DBH_CM, color = Zone, fill = Zone)) +
   geom_density() + 
   facet_grid(rows = vars(eastingBin_f), cols = vars(northingBin)) + 
   scale_color_pomological() +
   scale_fill_pomological() +
   ...
   labs(x = "Diameter at Breast Height (cm)", 
        y = "Frequency",
        title = "Diameter at Breast Height Distributions in S6", 
        subtitle = "Diversity of tree width is even across all regions.") +
   theme(strip.background = element_blank(),
         strip.text = element_blank(), 
         panel.background = element_blank(), 
         panel.grid = element_line(colour = 'grey92'),
 ...
 ggsave(paste0(fig_save, "S6DBHDistribution.png"), width = 10, height = 7)
```
...
```{r}
 ...
 ggplot(data = merged_data, aes(x = basalArea_m2, y = avg_depths, color = prop_Deciduous)) + 
   geom_point(size = 4) + 
   facet_grid(cols = vars(WATERSHED)) + 
   labs(x = "Basal Area (m2)", 
...
```{r}
ggplot(data = allData, aes(x = OLS.Prediction.Ring.5, y = avg_depths, color = prop_Deciduous)) + 
  geom_point(size = 4) + 
  facet_grid(cols = vars(WATERSHED)) + 
  labs(x = "Ring 5 Winter LAI", 
       y = "Average winter snow depth (m)",
       title = "Snow predictions from LAI only",
       subtitle = " ") +
  theme(#strip.background = element_blank(),
        #strip.text = element_blank(), 
        panel.background = element_blank(), 
