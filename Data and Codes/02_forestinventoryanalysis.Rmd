...
```{r}
 ...
 forestInv_sd = forestInv %>%
   group_by(SITE) %>%
   summarise(across(starts_with(c('D')), sd))
 ...
 forestInv_prop = forestInv %>%
  ...
  forestInv$OAI = (forestInv$basalArea_m2*10000)/(forestInv$DIST_M)
  forestInv_OAI = forestInv %>%
    group_by(SITE) %>% 
    summarise(across(starts_with('OAI'), sum))
 ...
 merged_forest = merge(forestInv_avg, forestInv_sd, by = "SITE")
 merged_forest = merge(merged_forest, forestInv_counts, by = "SITE")
 ...
 merged_forest = merge(merged_forest, forestInv_OAI, by = 'SITE')
 ```

 ```{r, echo = false}
 #Rename columns
 names(merged_forest)[2] <- "DIST_FT"
 names(merged_forest)[3] <- "DIST_M"
 names(merged_forest)[4] <- "DBH_IN"
 names(merged_forest)[5] <- "DBH_CM"

 names(merged_forest)[6] <- "DIST_FT_SD"
 names(merged_forest)[7] <- "DIST_M_SD"
 names(merged_forest)[8] <- "DBH_IN_SD"
 names(merged_forest)[9] <- "DBH_CM_SD"
 ```
